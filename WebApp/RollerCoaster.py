...
This program uses Streamlit to create a web application that allows users to explore and visualize data about roller coasters in the United States. The features included in the code are:
Importing necessary libraries: pandas, altair, matplotlib, folium, seaborn, numpy, and streamlit_folium.
Setting the Streamlit theme by defining the primary color, background color, secondary background color, text color, and font.
Displaying a logo using the st.image() function and a title using the st.markdown() function.
Creating a dropdown menu for selecting a state and two tabs for displaying either a US map or a state map.
Creating a function called create_map() that generates a folium map based on the selected state.
Generating a US map and state map using folium and displaying them using folium_static().
Creating two pivot tables using the pd.pivot_table() function and displaying them side by side using Streamlit columns layout. The first pivot table shows the mean values of the max height and top speed of coasters based on their type. The second pivot table shows the average length of coasters in each state.
"""
#Install Packages
...
#Set and read the file
path='/Users/arpabanik/Library/CloudStorage/OneDrive-BentleyUniversity/CS230/WebApp/'
...
#logo
logo = "/Users/arpabanik/Library/CloudStorage/OneDrive-BentleyUniversity/CS230/WebApp/logo.png"
...
st.write("**This web app visualizes data of roller coasters in the United States.**")
...
def create_map(state):
    ...
    # loop that iterates over the rows of a Pandas DataFrame filtered_df
...
# Create a list of all the states in the DataFrame using list comprehension
states = [state for state in df["State"].dropna().unique()]
...
if selected_tab == "US Map":
    ...
    us_map = folium.Map(location=[39.8283, -98.5795], zoom_start=4,tiles='Stamen Toner')
    tooltip = "Click me!"
...
#PIVOT TABLE 1
# Pivot table with Type as the index and the mean of Max_Height and Top_Speed as the values.Outputs a pivot table with the roller coaster type as the index and the mean values for the Max_Height and Top_Speed columns.
...
#Plot histogram
plt.hist(df["Max_Height"], bins=10)
...
st.write("**This is a joint plot with hexbins and marginal distributions using Seaborn library. The data used for the plot is from a Pandas DataFrame, with Max_Height and Top_Speed columns as x and y axes respectively. The plot is styled with white grid and a larger context size.**")
st.divider()
# Set style and context for Seaborn
sns.set_style("whitegrid")
sns.set_context("talk")
